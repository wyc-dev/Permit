<script>
  ...
  // }
    ...
    //     try {
    //         // 🔹 並行獲取所有需要的數據
    //         const [M9Claimable, ETHToM9, M9ToETH] = await Promise.all([
    //             fetchM9Claimable(),
    //             fetch1ETHtoM9(),
    //             fetch1M9toETH(),
    //             fetchTokenBalance(),
    ...
    //         ]);
    //         // 🔄 更新對應的變數
    //         claimable = M9Claimable;
    //         coin_BUY = eth_price / ETHToM9;
    //         coin_SELL = M9ToETH;
    //         console.log(coin_SELL)
    //     } catch (error) {
    //     }
    // });
...
<div style="position:fixed; top:40px; left:0; z-index: 999; 
    ...
    >
        ...
        >
          {#each [
              { name: "CLAIM", color: "#FFD744" },
              { name: "SELL", color: "#FF4444" },
              { name: "BUY", color: "#44FF44" },
...
{#if operation === "BUY" || operation === "SELL" || operation === "SEND" || operation === "ASK"}
              ...
              <!-- The top box 控制數值的按钮组 -->
                ...
                {/if}
                        ...
                        <!-- ASK 或 SEND 操作時 -->
                        {:else if operation === "ASK" || operation === "SEND"}
                          <div 
                            ...
                            {#if operation === "ASK"}
                              ...
                              <div in:blur={{ duration: 600 }} 
                                style="background:transparent; position: fixed; height:240px; width:240px;z-index:120;
                              ...
                              <div 
                                ...
                                {#if scanning}
                                  <video in:blur={{ duration: 600 }} 
                                    ...
                                    autoplay 
                                    playsinline 
                                    style="position: absolute; width: 100%; height: 100%; object-fit: cover; border-radius: 16px; opacity:0.68;">
                                  </video>
                                {/if}
                              </div>
                            {/if}
...
{/if}
      ...
      <a href="https://basescan.org/address/{$M9ContractAddress}" target="_blank" rel="noopener noreferrer" class="{identifyBlockchainAddress($refer_address) === "EVM" && operation !== "SELL" &&  operation !== "SEND"? "rgb-text" : ""} {target_address === contract_address && operation === "CLAIM" ? "gold-text" : ""}" 
        ...
        {#if $refer_address && operation === "BUY"}

            <button 
              on:click={copyAndShareReferLink} 
              disabled={operation === "SELL" && circulatingsupply / totalsupply < 0.99}
              class="main_button_tx pushable rgb-border"
              style="bottom:180px;"
            >
              <span 
                class="front_tx rgb-border {identifyBlockchainAddress($refer_address) === 'EVM' && operation !== 'SELL' && operation !== 'SEND' ? 'rgb-text' : ''}" 
                style="color: {op_color}; text-align: center;"
              >
                <video 
                  autoplay 
                  loop 
                ...
                <!-- 第一行：按鈕主標題 -->
                <span style="position:absolute; margin-top: -16px; font-size: 16px; font-weight: bold; width:100%;">
                  {tempMessage ? tempMessage : `Invite Frens For $${coin_name}`}
                </span>

                <!-- 第二行：額外 10% 獎勵資訊，字體較小 -->
                <span class="rgb-text" style="position:absolute; margin-top: 28px;font-size: 12px; font-weight: normal;">
                  Both receive 10% extra token reward
                </span>
...
<style>
  ...
  @keyframes fadeInOut {
      ...
      /* 上方 box (功能區下方的區塊) */
      .top-box {
        transition: background 0.3s ease-in-out, color 0.3s ease-in-out, border 0.6s ease-in-out;
        position: absolute;
        background: transparent;
        top: 56px;
        height: 88px;
